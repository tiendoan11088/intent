...
AltiplanoUtilities.prototype.getFiberNameFromOntState = function (ontTarget) {
    ...
    if(!fiberName){
        ...
        if(ontIntentConfigJSON && ontIntentConfigJSON["fiber-name"] && ontIntentConfigJSON["fiber-name"][0]){
            if (ontIntentStateJSON && ontIntentStateJSON["detected-fiber-name"] && ontIntentStateJSON["detected-fiber-name"][0]
                && ontIntentConfigJSON["fiber-name"][0] != ontIntentStateJSON["detected-fiber-name"][0]) { // In ont movement case
                fiberName = ontIntentStateJSON["detected-fiber-name"][0];
            } else {
                fiberName = ontIntentConfigJSON["fiber-name"][0];
            }
        } else if(ontIntentStateJSON && ontIntentStateJSON["detected-fiber-name"] && ontIntentStateJSON["detected-fiber-name"][0]){ // In ont easy start case
            fiberName = ontIntentStateJSON["detected-fiber-name"][0];
        } 
 ...
 */
 ...
 AltiplanoUtilities.prototype.getSubSetOfUsedProfilesInJsonFormat = function(intentType, target, intentProfileInputVos, storeInIntentScope) {
             ...
             if (intentProfileInputVos.length > 0) {
                 for (let index = 0; index < intentProfileInputVos.length; index++) {
                     ...
                     if (profileVo && profileVo.getProfileType() && profileVo.getSubtype() &&
                         profilesFromIntentScope[profileVo.getProfileType()] && profilesFromIntentScope[profileVo.getProfileType()][profileVo.getSubtype()]) {
                         ...
                         profilesToBeRetrieved.add(profileVo);
                     }
                 }
             } else {
     ...
     if (!profilesFromIntentScope || (profilesFromIntentScope && profilesToBeRetrieved && profilesToBeRetrieved.length > 0) || (intentProfileInputVos && intentProfileInputVos.length == 0)) {
         ...
         if (apUtils.getContentFromIntentScope("bulkSync") && usedProfilesJson && Object.keys(usedProfilesJson).length > 0 && parsedProfiles && Object.keys(parsedProfiles).length > 0) {
             Object.keys(parsedProfiles).forEach(function (profileType) {
                 Object.keys(parsedProfiles[profileType]).forEach(function (subType) {
                     if (!usedProfilesJson[profileType]) {
                         usedProfilesJson[profileType] = {};
                     }
                     usedProfilesJson[profileType][subType] = parsedProfiles[profileType][subType];
                 });
             });
...
AltiplanoUtilities.prototype.processProfilesGroupForOnuTemplate = function (eOnuResources, clockProfiles, tcToIdMappingProfiles, ontProfilesGroup, ltDeviceTopology, systemLoadProfilesContent, memoryUsageProfilesContent, tcaConfigJSON, powerSheddingProfileContent) {
    ...
    for (var templateIdx in onuTemplate) {
            ...
            ontProfilesGroup.forEach(function (profileGroup) {
                if (profileGroup && profileGroup["name"] == onuTemplate[templateIdx]["ont-profiles-group-id"]) {
		           ...
		           if (profileGroup["fec-err-profiles"]) {
                    ...
                    if (profileGroup["lldp-media-policy-profiles"]) {
                        // Get lldp-media-policy-profiles name from ont-profiles-group
                        profileGroup["lldp-media-policy-profiles"].forEach(function (lldpMedPolicyProfileName) {
                            if ((lldpMedPolicyProfileNames.length == 0) || (lldpMedPolicyProfileNames.length > 0 && lldpMedPolicyProfileNames.indexOf(lldpMedPolicyProfileName) == -1)) {
                                lldpMedPolicyProfileNames.push(lldpMedPolicyProfileName);
                            }
                        });
                    }
                }
        ...
        if (lldpMedPolicyProfileNames && lldpMedPolicyProfileNames.length > 0) {
            lldpMedPolicyProfiles[onuTemplate[templateIdx]["name"]] = {};
            lldpMedPolicyProfileNames.forEach(function (lldpMedPolicyProfileName) {
                var configedLldpMedPolicyProfiles = tcaConfigJSON["lldp-med-policy-profile"];
                if(configedLldpMedPolicyProfiles)
                {
                    configedLldpMedPolicyProfiles.forEach(function (configedLldpMedPolicyProfile) {
			            if(lldpMedPolicyProfileName == configedLldpMedPolicyProfile.name) {
                            lldpMedPolicyProfiles[onuTemplate[templateIdx]["name"]][lldpMedPolicyProfileName] = configedLldpMedPolicyProfile;
                        }
                    });
                }              
            });
    ...
    onuTemplateProfiles = {"trafficClassProfiles" : trafficClassProfiles, "ptpCcsaProfiles" : ptpCcsaProfiles, "ptpDotProfiles" : ptpDotProfiles,
                           ...
                           "lldpMedPolicyProfiles" : lldpMedPolicyProfiles};                    
                    ...
                    if(nodeType.startsWith(intentConstants.LS_FX_PREFIX) || nodeType.startsWith(intentConstants.FAMILY_TYPE_LS_MF_LANT_A) || nodeType.startsWith(intentConstants.FAMILY_TYPE_LS_MF_LBNT_A_MF14_LMFS_A)){
                        ...
                        if(plannedTypesFromDevice){
                            ...
                            if(nodeType.startsWith(intentConstants.FAMILY_TYPE_LS_MF_LANT_A) || nodeType.startsWith(intentConstants.FAMILY_TYPE_LS_MF_LBNT_A_MF14_LMFS_A)){
                                isGroupMatch = true;
                                var deprecated = apCapUtils.getCapabilityValue(ihubdeviceTypeAndRelease.hwType, ihubdeviceTypeAndRelease.release, capabilityConstants.PORT_MAPPING_CATEGORY, capabilityConstants.PORT_MAPPING_DEPRECATED, portMappingJson[i], false);
                                if(deprecated){
                                    isGroupMatch = false;
                                }
                            }
...
AltiplanoUtilities.prototype.configureNrmNodeObject = function(deviceName, input, networkState, serviceResource){
    var target = input.getTarget();
    var request = utilityService.processTemplate(resourceProvider.getResource("internal/esQueryGetNrmNodeObject.json.ftl"), {"deviceName" : deviceName});
	var response = apUtils.executeEsHamSearchRequest(request);
    var resourcePath = intentConstants.DIRECTORY_INTERNAL + intentConstants.FILE_SEPERATOR + "nrmNodeObject.json.ftl";
    var resourceFile = resourceProvider.getResource(resourcePath);
    if (apUtils.isResponseContainsData(response)) {
        var hit = response.hits.hits[0];
        var nrmNodeObject = hit['_source'];
        var nrId = nrmNodeObject['nr-id'];
        var expectedModelName = nrmNodeObject['expected-model-name'];
        var resources = [];
        var nrmNodeObjectResources = nrmNodeObject["supporting-resources"];
        if(networkState != "delete"){
            for (var key in nrmNodeObjectResources) {
                var resourceId = nrmNodeObjectResources[key]["resource-id"];
        ...
        var template = utilityService.processTemplate(resourceFile, templateArgs);
        return [{"templateState" : "create", "template" : template}];
    }
}

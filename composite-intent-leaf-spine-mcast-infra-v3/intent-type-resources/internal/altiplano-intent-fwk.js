...
AltiplanoIntentHelper.prototype.synchronize = function (input, deviceNameFilter) {
    ...
    try {
        ...
        if(Object.keys(intentSensitiveKeys.sensitiveKeys).length > 0) {
            var objectMapOfPasswordList = requestContext.get("objectMapOfPasswordList")
    ...
    for (let inputListIndex=0; inputListIndex < inputList.length; inputListIndex++) {
        ...
        if (intentConfigObjectsForAllDevices && intentConfigObjectsForAllDevices[targetName]) {
            ...
            Object.keys(intentConfigObjectsForAllDevices[targetName]).forEach(function (deviceId) {
                if (updatedSyncResult[targetName] && updatedSyncResult[targetName][deviceId]) {
                    syncResult = updatedSyncResult[targetName][deviceId];
                } else {
                    if (updatedSyncResult[targetName] && Object.keys(updatedSyncResult[targetName]) && Object.keys(updatedSyncResult[targetName]).length > resultMapIndex) {
                        let key = Object.keys(updatedSyncResult[targetName])[resultMapIndex];
                        syncResult = updatedSyncResult[targetName][key];
                    }
                }
                                        ...
                                        if (typeof step.postStepSynchronize === "function") {
                                            if (step.postStepSynchronize.length === 1) {
                                                ...
                                                if (postSyncResult && postSyncResult.proceed === false) {
                                                    syncResult.setSuccess(false);
                                                    syncResult.setErrorCode(postSyncResult.errorCode);
                                                    syncResult.setErrorDetail(postSyncResult.errorMessage);
                                                }
                ...
                self.updateSyncResult(resultMap[targetName], syncResult);
            });
        }
    }
    return resultMap;
...
AltiplanoIntentHelper.prototype.syncDevice = function (deviceInfo, input, intentConfigObject) {
    ...
    if (apUtils.getContentFromIntentScope("bulkSync")) {
        ...
        let intentConfigObjectsForAllDevices = apUtils.getContentFromIntentScope("intentConfigObjectsForAllDevices") ? apUtils.getContentFromIntentScope("intentConfigObjectsForAllDevices") : {};
        let targetName = input.getTarget();
        if (intentConfigObjectsForAllDevices && !intentConfigObjectsForAllDevices[targetName]) {
...
AltiplanoIntentHelper.prototype.gatherInformationAboutDevices = function (deviceNames) {
    var result = [];
    let bulkSync = apUtils.getContentFromIntentScope("bulkSync");
    for (var i in deviceNames) {
        ...
        if (typeof deviceInfo == "object") {
            ...
            if (bulkSync) {
                ...
                var managerInfo = mds.getAllManagersWithDevice(deviceInfo);
            }
            ...
            if (bulkSync) {
                ...
                var mdsDeviceInfo = mds.getAllInfoFromDevices(deviceInfo);
            }
